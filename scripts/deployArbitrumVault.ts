...
import { ARBITRUM, ARBITRUM_GOERLI } from "../utils/chains";
import { getContract } from "../utils/contracts";
import readerAbi from "../abi/Reader.json";
import exchangeRouterAbi from "../abi/ExchangeRouter.json";
import vaultAbi from "../abi/Vault.json";
import tokenPriceConsumerAbi from "../abi/TokenPriceConsumer.json";
...
const main = async () => {
    ...
    const tokenPriceConsumer = new ethers.Contract("0x10e551AD38e77fb4a70f989B00cd0679E95e0d7c", tokenPriceConsumerAbi, deployer);
    const vault = new ethers.Contract("0x0F1Fe6741bBDBaE5456346e1F0ba838f3223a4C7", vaultAbi, deployer);
    console.log("vault", vault.address);
    console.log("tokenPriceConsumer", tokenPriceConsumer.address);
    ...
    await gmxPlugin.setRouterConfig(
        getContract(ARBITRUM, "ExchangeRouter"),
        getContract(ARBITRUM, "SyntheticsRouter"),
        getContract(ARBITRUM, "DepositVault"),
        getContract(ARBITRUM, "WithdrawalVault"),
        getContract(ARBITRUM, "OrderVault"),
        getContract(ARBITRUM, "SyntheticsReader"),
    );
    console.log("setRouterConfig");
    // console.log("address", getContract(ARBITRUM_GOERLI, "DataStore"));
    // console.log("deployer", deployer.address);
    const ReaderContract = new ethers.Contract(getContract(ARBITRUM, "SyntheticsReader"), readerAbi, deployer);
    console.log("reader", ReaderContract.address);
    const marketlist = await ReaderContract.getMarkets(getContract(ARBITRUM, "DataStore"),  0, 100);
    console.log("market list", marketlist);
    const ethUsdMarket = marketlist[1];
    ...
    // const wbtcMockAggregator = await deployNew("MockAggregator", [18, "wbtc", 1, "40000000000000000000000", 0, 0, 0, "40000000000000000000000"]);

    // console.log("wbtc address", wbtcMockAggregator.address);
